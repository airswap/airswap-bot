...
export const stats = async () => {
	...
	const weekAgo = Math.round(Date.now() / 1000 - 604800);

	result = await axios.post(subgraphURL, {
		query: `{
        swapERC20S(
          where: {senderAmountUSD_gt:${Math.round(
						Number(process.env.BIG_SWAP_MIN_VALUE) / 2,
					)} blockTimestamp_gt:${weekAgo}}
        ) {
          nonce
          senderAmountUSD
          signerToken {
...
🔹 **$${formatNumber(
		weeklyVol,
	)}** 7-day vol (${weeklyChangeLabel}) / $${formatNumber(weeklyFees)} fees
🔥 **$${formatNumber(largest.senderAmountUSD)}** ${signerTokenInfo.symbol}/${
