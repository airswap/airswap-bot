...
export class SwapERC20 {
  ...
  private listener = async (
    ...
    const signerTokenInfo =
      ...
      (await getTokenInfo(this.provider, fullSwap.signerToken)) ||
      defaultTokenInfo
    const senderTokenInfo =
      findTokenByAddress(fullSwap.senderToken, tokens) ||
      (await getTokenInfo(this.provider, fullSwap.senderToken)) ||
      defaultTokenInfo
    ...
    try {
      const usdtToken =
        USDT.find((token) => token.chainId === chainId) || USDT[0]
      swapValue = await getValue(
        this.provider,
        signerTokenInfo,
        fullSwap.signerAmount,
        senderTokenInfo,
        fullSwap.senderAmount,
        usdtToken
      )
    } catch {
    ...
    const details: SwapEventParams = {
      ...
      senderTokens: `${commify(Number(senderUnits).toFixed(4))} ${
        senderTokenInfo.symbol
      }`,
      signerTokens: `${commify(Number(signerUnits).toFixed(4))} ${
        signerTokenInfo.symbol
      }`,
      swapValue,
      feeValue: swapValue * this.config.get('PROTOCOL_FEE_RATE'),
    }
